...
const open2__default = /*#__PURE__*/_interopDefaultLegacy(open2);
...
function createFetchError(request, response) {
  ...
  const error = new FetchError$1(message);
...
async function listen(handle, opts = {}) {
  opts = defu2__default['default'](opts, {
    name: "server",
  ...
  const port = await getPortPlease.getPort(opts.port || process.env.PORT);
  let server;
  let url;
  if (opts.https) {
  ...
  if (opts.clipboard) {
    await clipboardy2__default['default'].write(url).catch(() => {
      opts.clipboard = false;
    });
  }
  if (opts.showURL) {
    const add = opts.clipboard ? chalk2__default['default'].gray("(copied to clipboard)") : "";
    console.log(`> ${opts.name} listening on ${chalk2__default['default'].cyan.underline(decodeURI(url))}`, add);
  }
  const getURL = (...path) => ufo.joinURL(url, ...path);
  if (opts.open) {
    await open2__default['default'](url).catch(() => {
    });
  }
  if (opts.autoClose) {
  ...
  return {
    ...
    server,
    close,
    $fetch: (request, opts2) => $fetch(request, {...opts2, baseURL: url})
  };
}
...
function getSelfSignedCert(opts = {}) {
  return util.promisify(selfsigned.generate)(opts.attrs, opts).then((r) => ({key: r.private, cert: r.cert}));
}
